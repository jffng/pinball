...
var Flipper = function(_xPos, _yPos, type, orientation) {
	...
	var sawFixtureDef = new Box2D.b2FixtureDef;	
	sawFixtureDef.shape = new Box2D.b2PolygonShape;
	...
	if(type == "left")  localCenter.Add( new Box2D.b2Vec2( - 4 / 5 * flipperWidth, 0 ) );
	if(type == "right") localCenter.Add( new Box2D.b2Vec2(   4 / 5 * flipperWidth, 0 ) );

	var circleBodyDef = new Box2D.b2BodyDef;
	circleBodyDef.position.x = localCenter.x;
	circleBodyDef.position.y = localCenter.y;
	circleBodyDef.type = Box2D.b2Body.b2_staticBody;

	var circleFixtureDef = new Box2D.b2FixtureDef;
	circleFixtureDef.shape = new Box2D.b2CircleShape( 0.1 );
